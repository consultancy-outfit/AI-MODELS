...
const providerIcons = new Map(LAB_LIST.map((p) => [p.name, p.icon]));
...
export default function Discover() {
  ...
  const trending = useMemo(
    () => trendingModels.filter((m) => !q || [m.name, m.lab, m.description].some((v) => v.toLowerCase().includes(q))),
    [q],
  );
  const releases = useMemo(
    () => newReleases.filter((m) => !q || [m.name, m.lab, m.description].some((v) => v.toLowerCase().includes(q))),
    [q],
  );
  const providers = useMemo(() => featuredProviders.filter((p) => !q || p.name.toLowerCase().includes(q)), [q]);
  const prompts = useMemo(() => recommendedPrompts.filter((p) => !q || p.toLowerCase().includes(q)), [q]);
  ...
  return (
    <SiteShell>
      <Container maxWidth="lg" sx={{ pb: 8 }}>
        {/* Hero Header */}
        <Box sx={{ mb: 6 }}>
          <Typography sx={{ color: '#C8622A', fontWeight: 600, fontSize: '0.875rem', mb: 1, letterSpacing: 0.5 }}>
            AI landscape, curated daily
          </Typography>
          <Typography variant="h1" sx={{ fontFamily: 'var(--font-syne)', fontWeight: 700, fontSize: { xs: '2rem', md: '3rem' }, mb: 3 }}>
            Discover New
          </Typography>
        ...
        <Box sx={{ mb: 8 }}>
          <Typography variant="h3" sx={{ fontFamily: 'var(--font-syne)', fontWeight: 700, mb: 4, fontSize: { xs: '1.4rem', md: '1.75rem' } }}>
            Trending This Week
          </Typography>
              ...
              {trendCards.map((card, index) => (
                <Grid key={index} size={{ xs: 12, sm: 6, lg: 3 }}>
                  <MotionCard
                    ...
                    sx={{ height: '100%', bgcolor: card.color, borderRadius: 2, border: '1px solid rgba(28,26,22,0.06)', cursor: 'pointer' }}
                  >
                    <CardContent>
        ...
        <Box sx={{ mb: 8 }}>
          ...
          <motion.div initial="hidden" whileInView="visible" variants={containerVariants} viewport={{ once: true }}>
            <Grid container spacing={2}>
              {releases.slice(0, 6).map((model) => (
                <Grid key={model.id} size={{ xs: 12, sm: 6, lg: 4 }}>
                  <MotionCard
                    whileHover={{ y: -6, scale: 1.02 }}
                    transition={{ duration: 0.3 }}
                    variants={itemVariants}
                    sx={{ height: '100%', bgcolor: 'rgba(255,255,255,0.88)', borderRadius: 2, border: '1px solid rgba(28,26,22,0.06)' }}
                  >
                    <CardContent>
          ...
          <motion.div initial="hidden" whileInView="visible" variants={containerVariants} viewport={{ once: true }}>
            <Grid container spacing={2}>
              {providers.map((provider) => {
                ...
                const icon = providerIcons.get(provider.name) ?? providerIconAliases.get(provider.name) ?? provider.name[0];
                return (
                      ...
                      <CardContent>
                        <Box
                          sx={{
                            ...
                            bgcolor: `${provider.color}22`, mb: 2,
                            display: 'grid', placeItems: 'center',
                            fontSize: '1.25rem', fontWeight: 700,
                          }}
                        >
                          <Box component="span" sx={{ lineHeight: 1 }}>{icon}</Box>
                        </Box>
              ...
              {prompts.map((prompt) => (
                <motion.div key={prompt} variants={itemVariants}>
                  <Button
                    ...
                    sx={{
                      bgcolor: 'rgba(255,255,255,0.88)', color: '#1C1A16',
                      border: '1px solid rgba(28,26,22,0.06)', borderRadius: 2,
                      px: 2.5, py: 1.2, textTransform: 'none', fontSize: '0.9rem', fontWeight: 500,
                      '&:hover': { bgcolor: '#C8622A', color: '#FFF', transform: 'translateY(-4px)', boxShadow: '0 8px 16px rgba(200,98,42,0.2)' },
